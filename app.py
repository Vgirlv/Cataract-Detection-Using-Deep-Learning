...
BASE_URL = _secret("BACKEND_URL", "http://127.0.0.1:8000")
...
def generate_pdf_report(patient_info, label, confidence, probs):
    ...
    BLACK  = colors.HexColor("#080900")
    LEMON  = colors.HexColor("#E8FF00")
    LIME   = colors.HexColor("#A8E600")
    GOLD   = colors.HexColor("#C9A800")
    DARK   = colors.HexColor("#1e2100")
    MUTED  = colors.HexColor("#6b7a00")
    WHITE  = colors.HexColor("#ffffff")
    GREEN  = colors.HexColor("#2ECC40")
    ...
    title_style = sty("title", fontName="Helvetica-Bold", fontSize=22,
                      textColor=LEMON, alignment=TA_LEFT, spaceAfter=2)
    sub_style   = sty("sub",   fontName="Helvetica",      fontSize=9,
                      textColor=MUTED, alignment=TA_LEFT, spaceAfter=0)
    sec_style   = sty("sec",   fontName="Helvetica-Bold", fontSize=8,
                      textColor=MUTED, alignment=TA_LEFT, spaceAfter=6,
                      spaceBefore=14, leading=12)
    body_style  = sty("body",  fontName="Helvetica",      fontSize=9.5,
                      textColor=WHITE, alignment=TA_LEFT, leading=15, spaceAfter=4)
    bullet_style= sty("bullet",fontName="Helvetica",      fontSize=9.5,
                      textColor=WHITE, alignment=TA_LEFT, leading=15,
                      leftIndent=14, spaceAfter=3)
    verdict_style = sty("verdict", fontName="Helvetica-Bold", fontSize=14,
                        textColor=accent, alignment=TA_CENTER, spaceAfter=4)
    note_style  = sty("note",  fontName="Helvetica-Oblique", fontSize=8.5,
                      textColor=colors.HexColor("#aaaaaa"), alignment=TA_CENTER)
    footer_style = sty("footer", fontName="Helvetica", fontSize=7.5,
    ...
    # ── Header bar ──────────────────────────────────────────
    hdr_data = [[
        Paragraph("👁  EYESHIELD", sty("h", fontName="Helvetica-Bold", fontSize=16,
                                       textColor=LEMON, alignment=TA_LEFT)),
        Paragraph(f"Report Date: {datetime.now().strftime('%d %b %Y, %H:%M')}",
                  sty("d", fontName="Helvetica", fontSize=8,
    ...
    # ── Patient info ─────────────────────────────────────────
    elems.append(Paragraph("PATIENT INFORMATION", sec_style))
    ...
    pat_data = [
        ["Full Name",  name,    "Patient ID", str(pid)],
        ["Age",        str(age), "Gender",    gender],
        ["Contact",    contact,  "",          ""],
    ]
    pat_tbl = Table(pat_data, colWidths=[W*0.15, W*0.35, W*0.15, W*0.35])
    pat_tbl.setStyle(TableStyle([
        ...
        ("GRID",         (0,0), (-1,-1), 0.3, colors.HexColor("#2a3000")),
        ("ROUNDEDCORNERS",(0,0),(-1,-1), [4,4,4,4]),
    ]))
    elems.append(pat_tbl)
    ...
    verd_data = [[Paragraph(verdict_text, verdict_style)]]
    verd_tbl  = Table(verd_data, colWidths=[W])
    verd_tbl.setStyle(TableStyle([
        ("BACKGROUND",   (0,0), (-1,-1), colors.HexColor("#1a1e00")),
        ("LINEABOVE",    (0,0), (-1,-1), 2, accent),
        ("LINEBELOW",    (0,0), (-1,-1), 2, accent),
        ("TOPPADDING",   (0,0), (-1,-1), 12),
        ("BOTTOMPADDING",(0,0), (-1,-1), 12),
    ]))
    elems.append(verd_tbl)
    elems.append(Spacer(1, 8))

    # Probability table
    prob_data = [
        [Paragraph("METRIC", sty("ph", fontName="Helvetica-Bold", fontSize=8,
                                 textColor=MUTED, alignment=TA_LEFT)),
         Paragraph("VALUE", sty("ph2", fontName="Helvetica-Bold", fontSize=8,
                                textColor=MUTED, alignment=TA_RIGHT))],
        ["Overall Confidence",  conf_pct],
        ["Cataract Probability", cat_pct],
        ["Normal Probability",   nor_pct],
    ]
    prob_tbl = Table(prob_data, colWidths=[W*0.6, W*0.4])
    prob_tbl.setStyle(TableStyle([
        ("BACKGROUND",   (0,0), (-1,-1), DARK),
        ("BACKGROUND",   (0,0), (-1,0),  colors.HexColor("#0f1200")),
        ...
        ("LINEBELOW",    (0,0), (-1,-2), 0.3, colors.HexColor("#2a3000")),
        ("GRID",         (0,0), (-1,-1), 0.3, colors.HexColor("#2a3000")),
    ]))
    elems.append(prob_tbl)
    ...
    elems.append(HRFlowable(width=W, thickness=0.5, color=colors.HexColor("#2a3000")))
    elems.append(Spacer(1, 6))
    elems.append(Paragraph(
        "EyeShield v2.0  ·  AI Cataract Detection  ·  For research & educational use only  ·  "
...
with st.sidebar:
    ...
    st.markdown("[API Docs →](http://127.0.0.1:8000/docs)", unsafe_allow_html=True)
    ...
    ADMIN_ACCESS = admin_password == _secret("ADMIN_PASSWORD", "eyeshield123")
    # ─────────────────────────────────────────────────────────────────────────
